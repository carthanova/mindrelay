...
async function tryInjectContextAsync(inputEl: HTMLElement, target: EventTarget): Promise<void> {
  ...
  // 1. Native host FTS5/BM25 search — session-aware deduplication
  ...
  if (freshHost.length > 0) {
    ...
    log("[MindRelay] Claude: host FTS5 →", toInject.length, "results")
  }

  // 2. Local TF fallback when host is not installed or returned nothing
  if (toInject.length === 0 && cachedTranscripts.length > 0) {
    ...
    if (toInject.length > 0) log("[MindRelay] Claude: local TF →", toInject.length, "results")
  }
